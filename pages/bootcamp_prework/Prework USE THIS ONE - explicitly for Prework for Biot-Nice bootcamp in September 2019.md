---
title: IBM Technical Sales ICP4I prework before Biot/Nice enablement
toc: false
...
---
**IMPORTANT** This Lab Guide has been modified explictly for use as Prework for the Bootcamp in Biot/Nice in September 2019. It includes some extra configuration steps for you to do, and directs you only to work with ACE, Event Streams and MQ. The sections on API Connect are still included, but you are advised below to skip over them.
...
1. As part of the "Special Preparation for Biot/Nice Bootcamp" you will copy the following integration assets into your Developer Machine. Below is a description of each of the files that are relevant to this lab.
...
## Special Preparation for Biot/Nice Bootcamp

This section includes some preparation you will need to do:

1. Remove the running ACE Integration Server called `orders`
  -  Open the ICP4I Platform Navigator https://mycluster.icp/integration/
  -  Navigate via `ace1` (and if it does not open correctly then `Open ace1`) to the kist of deployed Integration Servers.
  -  Find the option to delete `orders` and delete it. You will be told to go the `Helm Releases` view. Do so.
  -  Use the search box to find `orders` and, under Actions far right, choose to `Remove` it.

2. Modify the Event Streams Helm release (to enable simple display of messages)
  - Open the ICP Portal  https://10.0.0.1:8443/console/dashboard .
  - Sign in and then, from the hamburger menu, select `Workload` -> `Helm Releases`.
  - Use the search box to find `es` and, under Actions far right, choose to `Upgrade` it.
  - From the Version drop-down list, select the highest chart version (V1.2.0 ?).
  ...
  - Down towards the bottom, find the Message indexing settings, and ensure that the box is un-ticked, to **disable** message indexing.
  - Click Upgrade.
  - You should immediately see a whirling timer and then you'll be taken back to the list of Helm releases. If nothing seems to happen, scroll to the top of the window - and you'll probably see the error message `t.text is not a function`. To correct this, take the following steps to remove the `helm-api` pods. NB You could also use the ICP Console to do all of the above, if you feel more comfortable taking that route.
    ...
    - Execute `kubectl get pods | grep helm-api` again to confirm that a  fresh pod has been created.
    - Then, back on the Helm Release, click `Upgrade` again.
  - At this point, your Event Streams Helm Release should have been upgraded.

3. Copy artefacts into the Developer Machine as follows:
  - In a Developer Machine Terminal session, navigate to directory _/home/student_ and create a subdirectory called _generateSecret_.
  - In a Developer Machine browser, navigate to https://github.com/sebastiansutter/techguides/tree/master/pages/cipdemo
  - Download the following five files into the new sub-directory _/home/student/generateSecret_:
    - generateSecret.sh - **chmod 777 generateSecret.sh** to make this rwx by everyone
    - mqsc.txt - **chmod 666 mqsc.txt** to make this rw by everyone
    - serverconf.yaml - **chmod 666 serverconf.yaml** to make this rw by everyone
    - setdbparms.txt - **chmod 666 setdbparms.txt** to make this rw by everyone
    - truststorePassword.txt - **chmod 666 truststorePassword.txt** to make this rw by everyone
  - Download the following file into directory _/home/student_):
    -  order.json - **chmod 666 order.json** to make it rw by everyone

4. Make sure that DOS-like newline characters generated by the above download do not cause any errors when running the script file:
  - In the Developer Machine Terminal session, navigate to directory _/home/studentgenerateSecret.
  - Enter the following, to install the dos2unix utility: `sudo apt install dos2unix`
  - Run that utility against the script file, to make sure it has UNIX-like newline characters: `dos2unix generateSecret.sh`

5. Edit the file _/home/student/.profile_ to contain the following 2 lines at the end:
````
...
---
**IMPORTANT** For the Biot/Nice Bootcamp in September 2019, do **not** perform this section. Instead, jump to the section headed **Prepare for Event Streams**.
...
2. Change to root thus `su - root` (providing the password for student: **Passw0rd!**).
3. In the `/root` directory, make a new directory, calling it whatever you like.  Change directories down into that new one.
4. In that new directory, clone this repository here on the `master` node:  `https://github.com/asimX/FS-CIP-Microservices`.  You do this on the Master Node because you need to load the docker images into ICP.
5. You might need to authenticate using your github.com account.  If you don't have one, you can sign up for your free one here at `github.com`
6. Go into the FS-CIP-Microservices directory.
7. Log into the ICP environment, thus:
...
1. Prepare the PEM file you downloaded earlier.
 - On the Developer Image, open the "Files" application.
...
The `orders` API provided by ACE will put one message onto a queue on a local Queue Manager (called **acemqserver**),
...
---
 **IMPORTANT** For the Biot/Nice Bootcamp in September 2019, do **not** perform the rest of this section. Instead, jump to the section headed **Modify the orders API within ACE**.

---

The `orders` API provided by ACE will **also** put one message onto a queue on a remote Queue Manager (called **mq**). This lab session uses those two Queue Managers, to illustrate the configuration differences between a remote and a local Queue Manager.
...
1. The third new node will put a message to an MQ queue on a remote Queue Manager (inside a separate pod).  **IMPORTANT** For the Biot/Nice Bootcamp in September 2019, you will **not** add this node.
...
---
**IMPORTANT** For the Biot/Nice Bootcamp in September 2019, your canvas will look slightly different. The instructions that follow are marked [Biot/Nice] where there are differences.
...
1. From the ACE palette, drag and drop an `HTTPHeader` node, [Biot/Nice] ~~two~~ one `MQOutput` node and a `KafkaProducer` node. Position them and connect them as shown in the screenshot above.
2. [Biot/Nice] Delete the connector between the `Input` node and the node that places the REST Request (`App Connect REST Request`). Wire a connector directly between `Input`'s **Out** terminal and `HTTP Header`'s **In** terminal. The reason for this is that the backend system that (`App Connect REST Request`) connects to is probably not available in Biot/Nice, so you have just configured this flow not to try to call it.
...
3. Follow these steps **_very carefully_**, to deploy the `orders` flow that you have just changed.
	- From the ACE Dashboard, in the `Servers` tab, select `Add server+` to start the process.
	...
	- For the `Target namespace`, select **ace**, because that was proposed by ACE earlier.
	...
	- For the `NodePort IP`, we recommend **orders.10.0.0.1.nip.io**. We recommend that the first part of the name (**orders** in this case) is identical to the `Integration Server name`, because there is a 1 to 1 relationship here.
...
---
**IMPORTANT** For the Biot/Nice Bootcamp in September 2019, do not check the standalone Queue Manager. Instead, jump to the section called **Review MQ Portion - ACE with MQ (Queue Manager acemqserver)**.
